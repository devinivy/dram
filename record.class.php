<?php
     ...
     * TODO
     * 
     * - investigate $this->record = $record (clone?)
     * o update, delete on recordcollection->record.  and generally taking advantage of multi queries.
     * o orderby, limit, (having?), those guys etc.
     * - default orderby
     * o paging stuff?
     * o write ->exists
     * - clear "pending" dynamics?
     * o extend the arrayiterator class to something sweet (then extend that to the recordcollection)
     * - manual SQL stuff, esp selecting other fields (that probably would be friends but aren't.)
     * - selecting all fields, or all plus some friends, etc.
     *      - esp to do a full join!
     * o syntax for selecting with joins
     *      o interaction between records.
     *      o abstracting table name for neighbor fields.
     * - make sure joins are not duplicated.
     * o give a way to execute straight SQL and still return a record object.
     * - set primary key for select, or no primary key.
     * 
     */
...
class Record extends ArrayIterator {
    ...
    public function __construct() {
        ...
        $primary_key_name = static::$info['primary_key'];
        
        static::$info['fields'][$primary_key_name]['primary_key'] = TRUE;
        
        $this->index_by($primary_key_field);
    ...
    public function select($fields = array()) {
        ...
        // only used for select routine, so let's reset.
        $this->fields[] = array();
        ...
        if ($fields_clause) {
            ...
            if ($success) {
                ...
                foreach ($raw_results as &$result_arr) {
                    ...
                    foreach ($result_arr as $field => $value) {
                    ...
                    if ($index) {
                        $results[$result_object->{$index}] = $result_object;
                    } else {
    ...
    private function fields_clause() {
        ...
        if (empty($this->fields)) {
            ...
            foreach ($this->joins as $join) {
                $join_tables[$join['neighbor']] = TRUE;
            }
            
            foreach ($join_tables as $join_table => $yes) {
                $fields[] = "$join_table.*";
            }
            ...
            if (!$selecting_index) {
                $this->fields[$this->index_by] = TRUE;
                $fields[] = $this->add_field($this->index_by);
            }
    ...
    public function index_by($index) {
    ...
    public function join($join) {
        ...
        if (!in_array($join_normalized, $this->joins))
            $this->joins[] = $join_normalized;
        
        return $this; 
    }    
    
    private function join_clause() {
